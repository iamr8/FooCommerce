...
public class LocalizedApplicationModelConvention : IApplicationModelConvention
{
    ...
    {
        _routePrefix = new AttributeRouteModel(new RouteAttribute(LanguageConstraints.LanguageKey));
    }
        ...
        foreach (var controller in controllers)
        {
            ...
            if (controllerTemplate.StartsWith("/"))
            {
                controller.Selectors.Add(new SelectorModel
                {
                    ...
                    {
                        Order = -1,
                        Template = $"/{{{LanguageConstraints.LanguageKey}:{LanguageConstraints.LanguageKey}}}{controllerTemplate}",
                ...
                foreach (var actionSelector in action.Selectors)
                {
                    ...
                    if (actionTemplate.StartsWith("/"))
                    {
                        newSelectors.Add(new SelectorModel
                        {
                            ...
                            {
                                Order = -1,
                                Template = $"/{{{LanguageConstraints.LanguageKey}:{LanguageConstraints.LanguageKey}}}{actionTemplate}",
