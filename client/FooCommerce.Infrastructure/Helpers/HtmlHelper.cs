...
public static class HtmlHelper
{
    ...
    public static HtmlString ReplaceHtmlByTagName(this string htmlDecodedText, Dictionary<string, Func<string, IHtmlContent>> tags)
    {
        ...
        foreach (var tagGroup in tags.Select(x => new { Id = x.Key, Tag = x.Value.GetTagBuilder() }))
        {
            try
            {
                ...
                var temp = htmlDecodedText.TryReplace(tag, tagDicName, out var tempHtmlDecodedText);
                if (!temp)
                    continue;
    ...
    {
        return htmlDecodedText.ReplaceHtmlByTagName(tags);
    }
    ...
    public static void MoveAttributesTo(this TagHelperOutput output, TagBuilder tagBuilder)
    {
        ...
        if (removingClasses.Any())
        {
            ...
            var finalClass = new StringBuilder();
            if (classes?.Any() == true)
            {
                removingClasses.ForEach(@class =>
                {
                    foreach (var _class in classes)
                    {
                        if (_class.Equals(@class))
                            continue;

                        finalClass.Append(_class);
                    }
                });
            }

            output.Attributes.SetAttribute("class", finalClass.ToString().TrimEnd());
        }
    }
    ...
    public static HtmlString ReplaceHtml(string html, params Func<string, IHtmlContent>[] tags)
    {
        if (string.IsNullOrEmpty(html))
            return new HtmlString(null);
        ...
        for (var i = 0; i < tags.Length; i++)
        {
            ...
            var temp = html.TryReplace(tag, i.ToString(), out var tempHtmlDecodedText);
            if (!temp)
                continue;
    ...
    private static bool TryReplace(this string html, TagBuilder tag, string tagName, out string editedHtmlText)
    {
        editedHtmlText = html;
