...
public class _TokenService : ITokenClient
{
    private readonly HttpClient _httpClient;

    private readonly ILogger<_TokenService> _logger;

    public _TokenService(HttpClient httpClient, ILogger<_TokenService> logger)
    {
    ...
    public async Task<DateTime?> GenerateAsync(Guid userCommunicationId, TimeSpan interval)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsync("api/token/index",
    ...
    public async Task<TokenStatus> ValidateAsync(Guid userCommunicationId, string code)
    {
        try
        {
            ...
            var response = await _httpClient.PutAsync("api/token/index",
                new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json"));
